...
1. [x] **Scaffold folders & package inits**
...
3. [x] **`.streamlit/config.toml` + `secrets.toml.example`**
...
4. [x] **`core/config.py` — Settings resolver**
...
7. [x] **`docker-compose.yml`**
   ...
   *Verified: `docker compose up -d`, then `psycopg.connect("postgresql://healthtracker:healthtracker@localhost:5432/healthtracker")` + `SELECT 1` — connected OK.*

8. [x] **`app.py` — minimal landing page**
   Per `design.md` #6: page config, title, one-line description, themed. No DB calls.

9. [x] **`tests/test_app_smoke.py`**
...
10. [x] **`tests/test_db_connection.py`**
    ...
    *Implementation note: used a root `conftest.py` with `os.environ.setdefault("DATABASE_URL", ...)` instead of `pyproject.toml`'s `[tool.pytest.ini_options]` — plain pytest ini options can't set env vars without an extra plugin (e.g. `pytest-env`), which wasn't in the approved dependency list. `conftest.py` achieves the same "local default, CI overrides" behavior with no new dependency.*
...
11. [x] **Alembic init**
    ...
    *Verified: `alembic upgrade head` ran cleanly; `information_schema.tables` shows only `alembic_version` — no app tables, as expected.*

12. [x] **`.github/workflows/ci.yml`**
    ...
    *Validated locally: ran the same three commands (`ruff check .`, `mypy .`, `pytest --cov`) against the same dependency set the workflow installs, on a matching DATABASE_URL shape. Not pushed/run on GitHub Actions yet — that happens once the branch is pushed in `/phase-ship`; YAML was also eyeballed against GitHub Actions' documented schema (checkout@v4, setup-python@v5, services.postgres with a healthcheck).*

13. [x] **`docs/deployment.md` stub**
    Placeholder noting deployment is finalized in phase 09; not blocking for this phase.

14. [x] **Full local verification pass**
    ...
    check formally.
    *Result: `ruff check .` — all checks passed. `mypy .` — no issues in 14 source files. `pytest` — 6/6 passed, 88% line coverage (above the 80% threshold).*
